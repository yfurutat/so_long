...
/*   Updated: 2023/12/20 09:11:14 by yfurutat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "so_long.h"

void	pave_path(char **map, size_t x, size_t y, size_t *cnt)
{
	if (map[y][x] == 'C')
		(*cnt)++;
...
char	**mapdup(char **map, size_t height)
{
	char	**dup;
	size_t	i;
...
void	check_path(t_data *data)
{
	...
	free_map(map_copy);
	if (exit_chr != 'F' || cnt < data->collectibles)
